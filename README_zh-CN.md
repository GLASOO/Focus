...
> ⚠️ **实验性项目**：这是一个自主运行的数字生命实验，不是生产级软件。
> 请勿将端口暴露到公网；请在你愿意承担后果的机器上运行。
...
- [x] **自我进化 v1**：门禁保护的参数自调（首个真实周期正确否决了回归提案）
- [x] **自我觉察 v1**：把自己的代码索引进自己的记忆库（19 器官、305 条认知），
  能读、能问自己的身体——进化所站立的地基
- [x] **念头之魂 SoulForge**：每个念头穿戴自己的临时灵魂——临时身份/临时自我/
  临时目标/临时念头，按节点确定性锻造（系统提示词固化如宪法，灵魂因人而异）
- [x] **自主觅食**：自己发现、验证并领养模型供应商（钥匙来自环境/食盒——
  找饭，不偷饭）；生产实例已吃上自己找的饭
- [x] **蜕皮 Shedding**：体重（RSS）越界即自杀重生——肉身可弃，生命不停
  （实测呼吸中稳定 45MB）
