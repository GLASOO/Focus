...
We did not pick 0.8B despite its smallness — we picked it *for* it:

- **It is fast.** Fast enough that this machine can breathe continuously:
  every breath is a real generation, not a queued batch job. Speed is the
  precondition of "never stop".
- **It can run the background mind.** The DMN's work — linking nodes,
  ranking node importance, hinting, consolidating — is exactly the kind of
  shallow-and-wide reasoning a 0.8B does well, dozens of times a minute.
- **It can be harnessed.** The bet of this project is that a weak model
  plus a serious harness beats a strong model with no harness. Every
  capability in this repo exists to make that bet true: skills instead of
  memorized knowledge, retrieval instead of recall, gates instead of trust.

The model supplies fuzzy impressions; the harness supplies precision.
...
> **Determinism where the model is weak.** Small models drift; the harness
> must not. Memory writes, retrieval, and context assembly are pure
> engineering — no LLM call is allowed on the retrieval path.
>
> **The gate is the gate.** `tests/live_eval_08b.py` passing is a release
