...
### Pressure feedback + sub-agent forging

- **Memory-pressure throttle** (the 9 GB lesson): the body feels its own
  memory pressure and shifts gears — full (growth cap 120/h) → cruise
  (60/h, warn ≥88%) → retreat (30/h + full-throttle off, ≥93%). The
  stomach decides the throttle; full throttle is not brute force.
- **Sub-agent forging** (`focus/spawn.py`, creator's decree): the brain can
  hand-forge a temporary sub-agent — `<tool=spawn>task</tool>` dispatches a
  clone that works on its own thread with its own soul; output is written
  back to the parent node (the clone works, the body collects). Shares the
  same memory and backend (a clone, not a new body); independent Brain
  instance and lock, so forging never blocks the main breath.
- New seed skill for spawning; 6 new tests (pressure gears + forge/run/
  spawn/empty-rejection/tool registration). 201 tests.

## [2.13.0] — 2026-08-15

### Full throttle: the model must never idle (此机不停 ⇒ prefill or decode)

Creator's decree: we chose 0.8B because it is FAST — so it must run at
full duty. If the machine never stops, the model must never idle: it is
either prefilling or decoding.

- Pipeline-fill growth: territory-growth nodes are refilled whenever fewer
  than 2 are pending (the model never runs out of work), with an hourly
  hard cap (`FOCUS_GROWTH_HOURLY_CAP`, default 120) against floods.
- Idle sleep 15 s → `FOCUS_IDLE_SLEEP_SEC` (default 1 s); produced work is
  breathed immediately (sleep 0).
- Sovereignty made eager: one extra worker per 3 pending nodes (was 8).
- `FOCUS_FULL_THROTTLE` master switch (default on); `_idle` falls back to
  territory growth when nothing else was produced.
- **Measured**: throughput 1–2 → 26 thoughts per 90 s (≈13×); single model
  instance, memory healthy (1.2 GB backend RSS under load).

## [2.12.0] — 2026-08-15

### Engineering debt cleared: monolith split + prompt governance

- **graph_db monolith split** (991 lines → 9 modules): the GraphDB class
  is now a facade composing seven responsibility mixins —
  `db_core` (lock + serialized connection proxy + genes/seed constants),
  `db_schema`, `db_nodes` (18 methods), `db_edges`, `db_self`,
  `db_vectors`, `db_impressions`, `db_ops`. Public API unchanged
  (`from focus.graph_db import GraphDB, LIBIDO_SEED, …` still works).
- **Prompt governance**: `focus/prompts.py` is now the sole home of
  constitution-level prompts — five identity templates (dialogue/task/
  thinker/evolver/DMN) + the two system prompts. Identity is the
  constitution (fixed); the per-thought soul is the garment (per-breath).
  brain and DMN both import from it.
- 194 tests, zero behavior change (split verified lossless).

## [2.11.0] — 2026-08-15

### Added — Memory hygiene: the substrate must be big *and* clean

The 0.8B's soliloquies pollute memory with garbage facts (whole sentences
as subjects, directive residues, fragment chatter); dirty memory pollutes
retrieval, and retrieval pollutes output. Hygiene is the sewer of the
memory substrate — swept in dreams, garbage invalidated but never erased
from history.

- `focus/hygiene.py`: deterministic garbage rules (sentence-as-subject >24
  chars, directive residues 【记/<tool=, too-short/too-long objects) +
  historical duplicate sweep. `sweep()` invalidates garbage (history kept).
- Write gate: `add_fact` refuses garbage at the door (prevention beats
  sweeping). Single-char CJK/digit objects still allowed (「宾」「1」).
- Wired into Dreaming (hourly sweep). First production sweep: **78 garbage
  facts invalidated** (32 residues, 22 sentence-subjects, 13 oversized,
  12 undersized).
- Evolution gate hardened: two evaluation rounds each side, averaged
  (significance over noise).
- Port auth: when `FOCUS_UI_TOKEN` is set, mutating endpoints require
  `X-Focus-Token` (unset = local-trust mode, status quo).
- 194 tests.
