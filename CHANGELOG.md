...
- **Web curiosity module** (`focus/web.py`, self-evolved by the agent itself
  as 文昌夫人): zero-dependency web learning pipeline, `FOCUS_WEB=0` to
  disable. The build-machines gene expressing itself.
- Tool sandbox hardening (regex FORBIDDEN_PATTERNS: pipe-to-shell, eval/exec,
  block devices) — also self-evolved.
