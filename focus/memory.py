...
class MemoryHarness:
    ...
    def _vector_search(self, query: str, k: int = 10) -> list:
        """向量余弦路：query 嵌入 vs 活事实嵌入（阈值 0.3）。"""
        if self._embedder is None:
            return []
        try:
            import numpy as np
            ...
            rows = self.db.conn.execute(
                ...
                "AND embedding IS NOT NULL LIMIT 200").fetchall()
            scored = []
            for r in rows:
                ...
                cos = float(np.dot(qv, v) /
                            (qn * (float(np.linalg.norm(v)) + 1e-9)))
                if cos > 0.3:
                    scored.append((cos, dict(r)))
            scored.sort(key=lambda x: -x[0])
            return [f for _, f in scored[:k]]
        except Exception:
            return []

    # ── L2 Wiki（M3：DMN Dreaming 汇编） ────────────
