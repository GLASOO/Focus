...
class MemoryHarness:
    ...
    def add_fact(self, subject: str, predicate: str, obj: str,
        ...
        if not (subject and predicate and obj):
            return ""
        # 2026-08-15 记忆卫生写入端：垃圾当场拒收（预防胜于清扫）
        try:
            from .hygiene import is_garbage
            if is_garbage(subject, predicate, obj):
                return ""
        except Exception:
            pass
    ...
    def _query_terms(query: str) -> list:
        """意图切分（检索的"理解意图"层，确定性）：
        去停用词 → 按标点/空格切段 → 中文段再拆 2-4 字滑窗。
        小模型+大记忆的检索必须宽容：整句命中是偶然，词元命中是必然。"""
        import re as _re
        stop = {"的", "了", "是", "在", "和", "与", "有", "多少", "是什么",
                "请问", "请", "吗", "呢", "什么", "哪个", "怎么", "几", "the",
                "a", "an", "is", "of", "what", "how", "many"}
        segs = _re.split(r"[，。？！、；：\s,.?!;:]+", (query or "").lower())
        terms = []
        for s in segs:
            s = s.strip()
            ...
            if _re.search(r"[\u4e00-\u9fff]", s):
                # 中文策略：剥首尾停用词后的整段 + 顺序二元组。
                # 不盲目滑窗——4字碎片大多是无用噪声，会把有用的
                # 二元组挤出检索窗口（2026-08-15 实测教训）。
                core = s
                while core and core[0] in "".join(stop):
                    core = core[1:]
                while core and core[-1] in "".join(stop):
                    core = core[:-1]
    ...
    def search_memory(self, query: str, node_id: Optional[str] = None,
        ...
        try:
            ...
            pass
        # 2. LIKE 兜底（中文短事实主力——词元联合检索，意图切分 + IDF）
        # IDF（2026-08-15）：高频词（如"focus"几乎命中全部事实）必须降权，
        # 否则意图词被压制——检索要懂"区分度"，如同认知中的选择性注意。
        import math as _m
        total_facts = c.execute(
            "SELECT COUNT(*) c FROM facts WHERE invalid_at IS NULL"
        ).fetchone()["c"] or 1
        terms = self._query_terms(query) or [query]
        seen_ids = set()
        for t in terms[:8]:
            like = f"%{t}%"
            df = c.execute(
                "SELECT COUNT(*) c FROM facts WHERE invalid_at IS NULL AND "
                "(subject LIKE ? OR predicate LIKE ? OR object LIKE ?)",
                (like, like, like)).fetchone()["c"]
            if df == 0:
                continue
            idf = _m.log(1 + total_facts / (1 + df))
            strength = (1.0 + 0.4 * max(0, min(len(t) - 2, 2))) * idf
            # 列优先级排序：谓词命中 > 主语 > 宾语（谓词是事实的"骨架"）
            for r in c.execute(
                    "SELECT * FROM facts WHERE invalid_at IS NULL AND "
                    "(subject LIKE ? OR predicate LIKE ? OR object LIKE ?) "
                    "ORDER BY (predicate LIKE ?) DESC, (subject LIKE ?) DESC "
        ...
        # 2026-08-15：valid_at 是 UTC（datetime('now')），必须按 UTC 算年龄，
        # 否则新事实在本地时区下被误算成"未来/0天"，排序失真
        import calendar
        ...
        for fid, base in cand.items():
            ...
            try:
                va = calendar.timegm(
                    time.strptime(f["valid_at"], "%Y-%m-%d %H:%M:%S"))
                age_days = max(0.0, (now_utc - va) / 86400.0)
            except Exception:
                age_days = 0.0
