...
def decide(db) -> dict:
    ...
    # ── 决策零：内存压力油门（2026-08-15，9GB 事故教训） ──
    # 打满不是蛮干：胃胀了就收油门。三档：full/cruise/retreat
    try:
        from .hygiene import mem_pressure
        pressure = mem_pressure()
    except Exception:
        pressure = 0.5
    from . import config as _cfg
    if pressure >= getattr(_cfg, "MEM_PRESSURE_RETREAT", 0.93):
        throttle, cap = "retreat", 30
    elif pressure >= getattr(_cfg, "MEM_PRESSURE_WARN", 0.88):
        throttle, cap = "cruise", 60
    else:
        throttle, cap = "full", getattr(_cfg, "GROWTH_HOURLY_CAP", 120)
    if gov.get("throttle") != throttle:
        reasons.append(f"内存压力{pressure:.0%} → 油门 {throttle}（生长闸{cap}/h）")
    gov["throttle"] = throttle
    gov["pressure"] = round(pressure, 3)
    try:
        _cfg.GROWTH_HOURLY_CAP = cap           # 活配置：生长硬闸随压力变
        _cfg.FULL_THROTTLE = throttle != "retreat"  # 撤退时打满关闭
    except Exception:
        pass
