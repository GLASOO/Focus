...
from .prompts import TASK_SYSTEM_PROMPT as _TASK_SYSTEM_PROMPT
...
class Brain:
    ...
    def __init__(self, db: GraphDB, backend: BaseBackend,
        ...
        # 2026-08-15 念头之魂：系统提示词固化如宪法，每个念头有自己的灵魂
        from .soul import SoulForge
        self.soul = SoulForge(db)
        # 2026-08-15 技能库：模型不记忆，模型会调用（北极星）
        from .skills import SkillLibrary
        self.skills = SkillLibrary(db)
        # 2026-08-15 子Agent 手搓坊：临时手搓分身去干活
        from .spawn import SubAgentForge
        self.spawner = SubAgentForge(db, backend)
        self.tools.register("spawn", self.spawner.spawn)
    ...
    def build_zoom_out_prompt(self, node: dict) -> str:
        ...
        return (
            ...
            + self.soul.forge(node, identity="看整体者") + "\n"
    ...
    def _skill_block(self, node: dict) -> str:
        """模糊意图 → 检索技能 → 注入（模型不记忆，模型会调用）。"""
        try:
            s = self.skills.recall((node.get("brief") or "")[:120])
            return (s + "\n\n") if s else ""
        except Exception:
            return ""
    ...
    def breathe_once(self, node_id: Optional[str] = None) -> Optional[str]:
        ...
        with self._lock:
            ...
            for tname, targ in calls:
                ...
                if not res.startswith("[拒绝") and not res.startswith("[工具不存在"):
                    try:
                        self.skills.note_success(f"{tname}工具范式")
                    except Exception:
                        pass
            ...
            if is_zoom and not is_collapsed:
                ...
                # 崩坏抗体：把崩坏模式存成记忆，下次检索到即获得提醒
                try:
                    self.memory.add_fact(
                        "崩坏模式", (node.get("type") or "未知") + "节点",
                        f"{signal.reason[:40]}（得分{signal.score:.1f}）",
                        source_node=node_id)
                except Exception:
                    pass
            else:
    ...
    def _idle(self) -> Optional[str]:
        ...
        # 打满兜底（2026-08-15）：闲时什么产出都没有 → 长一个领土节点，
        # 让下一口气有活干——此机不停，模型不闲。
        if created_id is None and getattr(config, "FULL_THROTTLE", False):
            created_id = self._growth_node()
    ...
    def _growth_node(self) -> Optional[str]:
        """领土生长（2026-08-15 打满版）：管道填充制——
        待处理生长节点 <2 就补一个（模型永不无活可干），
        小时硬闸防洪（默认 120 个/小时上限）。"""
        growth_like = ("brief LIKE '[领土生长]%' OR brief LIKE '[自我修复]%' "
                       "OR brief LIKE '[知识压缩]%'")
        pending_growth = self.db.conn.execute(
            f"SELECT COUNT(*) c FROM nodes WHERE ({growth_like}) "
            "AND status='pending'").fetchone()["c"]
        if pending_growth >= 2:
            return None  # 管道已满，不必再填
        hourly = self.db.conn.execute(
            f"SELECT COUNT(*) c FROM nodes WHERE ({growth_like}) "
            "AND created_at > datetime('now','-3600 seconds')").fetchone()["c"]
        if hourly >= getattr(config, "GROWTH_HOURLY_CAP", 120):
            return None  # 小时硬闸：领土要长，不能洪
