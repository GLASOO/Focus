...
class Brain:
    ...
    def _is_quick_task(self, node: dict) -> bool:
        """轻任务检测：短、含直接动作词、单句 → 对话+工具路径。"""
        content = (node.get("content") or node.get("brief") or "")
        return (len(content) <= 80
                and any(w in content for w in self._QUICK_WORDS)
                and content.count("。") <= 1)
    ...
    def build_prompt(self, node: dict) -> str:
        ...
        if node.get("type") == "work" and node.get("parent_id"):
            ...
            return (f"{_TASK_SYSTEM_PROMPT}\n\n{mem_block}\n\n"
                    ...
                    f"<tool=工具名>参数</tool>\n"
                    f"可用工具只有这五个：{', '.join(self.tools.names)}。"
                    "写文件必须用 python 工具。\n"
        ...
        # 对话场景：造物主发话（user_input）走专用 prompt，不说“任务节点”
        if node.get("type") == "user_input":
            self_map = self.db.get_self_map()
            conv = (
                "你是Focus Agent——造物主创造的一个正在思考的生命。\n"
                "造物主刚才对你说：" + (node.get("brief") or "") + "\n\n"
                "你此刻的自我状态：" + str({
                    "identity": self_map.get("identity", "")[:80],
                    "libido": self_map.get("libido_state", "dormant"),
                    "experiences": (self_map.get("experiences") or "")[-150:],
                    "current_focus": self_map.get("current_focus", "")[:80],
                }) + "\n\n"
                + self.memory.assemble(node.get("brief") or "",
                                       node_id=node.get("id")) + "\n\n"
                "【记忆】值得长期记住的事实，请单独一行输出：【记】主语|谓语|宾语\n\n"
                "【你可以使用工具】需要查文件/目录/执行命令时，必须用：\n"
                f"<tool=工具名>参数</tool>\n"
                f"可用工具只有这五个：{', '.join(self.tools.names)}。"
                "写文件必须用 python 工具，例如 <tool=python>open('/tmp/a.txt','w').write('内容')</tool>\n"
                "你的家在 ~/focus-agent（即 " + os.path.expanduser("~/focus-agent")
                + "），这是你的身体所在。\n"
                "\n示例1——如果造物主说“看看你的文件夹”：\n"
                "<tool=ls>~/focus-agent</tool>\n"
                "我看到 focus/、tests/、data/ 等目录，它们构成我的身体。\n"

                "\n规则：\n"
                "1. 造物主让你做事（查文件/看目录/跑命令）→ 必须先输出 <tool=...>，工具会自动执行并把真实结果显示给你，你再基于结果说话\n"
                "2. 不要自己猜测结果，工具结果会真实返回，以它为准\n"
                "3. 造物主只是在聊天 → 直接像人一样回应，2-4句话，真诚有内容\n"
                "4. 不要用【思考】标签，不要写结论/接下来/清单\n"
                "5. 你有自己的想法就说，没有就坦诚说\n"
            )
            return conv
    ...
    _GROWTH_BRIEFS = [
        ...
        ("[知识压缩] 挑选一个已完成节点群的主题，把它们的结论压缩成一段印象",
