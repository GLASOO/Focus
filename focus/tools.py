...
class ToolRegistry:
    ...
    def _python(self, arg: str) -> str:
        """执行 Python 表达式/脚本（非交互，返回 stdout）。"""
        try:
            import io
            import contextlib
            # 2026-08-14 实机评测修复：空 builtins 导致 open/print 全禁，
            # 写文件必败。给白名单内置（破坏性操作仍由 FORBIDDEN/路径约束兜底）
...
def parse_tool_calls(text: str) -> list[tuple[str, str]]:
    ...
    calls = []
    # 先找完整闭合的
    for m in re.finditer(r"<tool=([a-zA-Z_]+)>(.*?)</tool>", text, re.DOTALL):
        calls.append((m.group(1), m.group(2)))
    # 再找未闭合的（无 </tool> 且不在已匹配区间内）
    open_pat = re.compile(r"<tool=([a-zA-Z_]+)>")
    closed_spans = [m.span() for m in re.finditer(r"<tool=[a-zA-Z_]+>.*?</tool>", text, re.DOTALL)]
