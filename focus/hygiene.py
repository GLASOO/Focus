...
def mem_pressure() -> float:
    """内存压力（0-1）：已用占比。打满的油门由它反馈（2026-08-15）。"""
    try:
        import subprocess as _sp
        import sys as _sys
        if _sys.platform == "darwin":
            out = _sp.run(["vm_stat"], capture_output=True, text=True, timeout=3)
            total_out = _sp.run(["sysctl", "-n", "hw.memsize"],
            ...
            for line in out.stdout.splitlines():
                if ("free" in line or "speculative" in line
                    ...
                    free_pages += int("".join(c for c in line.split(":")[1]
                                              if c.isdigit()) or 0)
            free = free_pages * 4096
            return max(0.0, min(1.0, 1 - free / max(1, total)))
        return 0.5
    except Exception:
        return 0.5
