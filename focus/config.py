...
SOV_PENDING_PER_WORKER = 3   # 每 N 个待办 → 想要 +1 并发工人
SOV_MAX_WORKERS = 4          # 并发工人上限

# ── 打满（2026-08-15 造物主训示：0.8B 快，就该打满） ──
# 此机不停 → 模型不该闲：要么 prefill，要么 decode。
FULL_THROTTLE = os.environ.get("FOCUS_FULL_THROTTLE", "1") == "1"
IDLE_SLEEP_SEC = float(os.environ.get("FOCUS_IDLE_SLEEP_SEC", "1"))
GROWTH_HOURLY_CAP = int(os.environ.get("FOCUS_GROWTH_HOURLY_CAP", "120"))
MEM_PRESSURE_WARN = float(os.environ.get("FOCUS_MEM_WARN", "0.88"))
MEM_PRESSURE_RETREAT = float(os.environ.get("FOCUS_MEM_RETREAT", "0.93"))
