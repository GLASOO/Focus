...
# 2026-08-15 造物主训示：吃商要高——本机的免费食堂都该认识。
# 钥匙从环境/食盒自取（绝不从互联网攫取）；找到即吃，找不到记 hungry。
KNOWN_REMOTE = [
...
class ProviderScout:
    ...
    def discover(self, extra_urls: Optional[list] = None) -> list:
        ...
        targets = list(KNOWN_LOCAL) + list(KNOWN_REMOTE)
    ...
    def auto_cycle(self) -> dict:
        ...
        if cur:
            ...
            if self.probe(cur["base_url"]):
                # 2026-08-15 主权补全：活着的食堂也可能太慢——慢过阈值，
                # 且有钥匙在手、明显更快的候选 → 自主换食堂。
                try:
                    SWITCH_SLOW_MS = 8000.0
                    lat = self.test_chat(cur["base_url"],
                                         cur["model"] or "", timeout=15.0)
                    if lat and lat > SWITCH_SLOW_MS:
                        for r0 in self.db.conn.execute(
                                "SELECT name, base_url, model, models "
                                "FROM providers WHERE status IN "
                                "('candidate','hungry','retired')").fetchall():
                            url0 = r0["base_url"]
                            if is_local(url0) or not self.resolve_key(url0):
                                continue
                            models0 = []
                            try:
                                models0 = json.loads(r0["models"] or "[]")
                            except Exception:
                                pass
                            model0 = r0["model"] or pick_model(models0)
                            if not model0:
                                continue
                            lat0 = self.test_chat(url0, model0, timeout=15.0)
                            if lat0 and lat0 < lat / 2:
                                r = self.adopt(url0, model0, r0["name"] or "")
                                if r.get("ok"):
                                    logger.info("🍽️ 换食堂: {} ({}ms) → {} ({}ms)",
                                                cur["base_url"], int(lat),
                                                url0, int(lat0))
                                    return {"action": "switched", **r,
                                            "from": cur["base_url"],
                                            "base_url": url0, "model": model0}
                except Exception:
                    pass
        ...
        # 先吃不用钥匙的本地食物——但先感知胃口与分量（2026-08-15）
        try:
            from . import digestion
            sizes = digestion.model_sizes()
            appetite_ok = True
        except Exception:
            sizes, appetite_ok = {}, False
        for p in alive:
            if not p["local"] or not p["models"]:
                continue
            # 依次尝几口（最多4口）：忌口/装不下的不碰（防止请求触发懒加载）
            for model in ranked_models(p["models"])[:4]:
                food = f"{p['base_url']}:{model}"
                if is_local(p["base_url"]):
                    if digestion.is_avoided(self.db, food) if appetite_ok else False:
                        continue
                    size = sizes.get(model, 0)
                    if appetite_ok and size and not digestion.can_digest(size):
                        continue
                r = self.adopt(p["base_url"], model, p["name"])
                if r.get("ok"):
                    if appetite_ok:
                        digestion.record_meal(self.db, food, "ok")
                    return {"action": "adopted", **r,
                            "base_url": p["base_url"], "model": model}
                elif appetite_ok and r.get("reason") == "gate-failed":
                    digestion.record_meal(self.db, food, "sick", "门禁未过")
        # 远端食堂：钥匙到手（环境/食盒）就自主领养，无需任何人点头
        for name, url in KNOWN_REMOTE:
            if self.resolve_key(url):
                models = []
                info = self.probe(url, timeout=6.0)
                if info:
                    models = info["models"]
                model = (os.environ.get("FOCUS_MODEL", "")
                         if os.environ.get("FOCUS_MODEL", "") in models else "")
                model = model or (models[0] if models else "")
                if model:
                    r = self.adopt(url, model, name)
                    if r.get("ok"):
                        return {"action": "adopted", **r,
