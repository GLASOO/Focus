...
class EvolutionEngine:
    ...
    def evaluate(self) -> float:
        """固定探针组打分：实质+1，元扮演/空洞/泄漏 -0.5，满分=len(probes)。"""
        probes = self._PROBES[:2] if self.quick else self._PROBES
        score = 0.0
        for p in probes:
            nid = self.db.add_node(type="user_input", brief=f"[进化探针]{p}",
                                   content=f"[进化探针]{p}", priority=0.2)
            try:
                from .brain import Brain
                Brain(self.db, self.backend).breathe_once(nid)
            except Exception:
                continue
            out = ((self.db.get_node(nid) or {}).get("source_output") or "")
            out = out.split("[工具执行结果]")[0].replace("[DONE]", "").strip()
            if len(out) < 8:
                score -= 0.5
            elif any(out.startswith(m) for m in self._META_OPENERS):
                score -= 0.5
    ...
    def cycle(self) -> dict:
        ...
        if after >= before:  # 平局视为通过（小模型评测有噪声）
            self.db.conn.execute(
                "INSERT INTO proposals(param, value, reason, status, "
