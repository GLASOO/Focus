...
USER_AGENT = "FocusAgent/1.0 (life-form agent; learning mode)"
...
def extract_readable(html: str) -> str:
    ...
    策略：
      1. 移除 script/style/nav/header/footer 标签及内容
      2. 提取 <p>, <li>, <h1>-<h3>, <code>, <pre> 内文本
      3. 清理 HTML 实体、多余空白
      4. 截断到 MAX_CONTENT_CHARS
    """
    if not html:
        return ""
    # 移除不需要的标签及内容
...
def web_search(query: str, n: int = MAX_SEARCH_RESULTS) -> list[dict]:
    ...
    try:
        ...
        # 退路：尝试 DDG JSON API（ddgs)
        return _ddg_json_search(query, n)

    # DDG Lite 表格结构：<table> <tr> ... <a href="URL">TITLE</a> ... SNIPPED
    results = []
    # 提取所有链接（跳过 DDG 内部链接）
...
class WebCuriosity:
    ...
    def explore(self) -> Optional[str]:
        ...
        # 4. 无论 0.8B 是否成功，把搜索摘要写入 Graph 作为知识节点
        brief = f"[网学] {topic}"
        summary_parts = [f"搜索「{topic}」"]
        for r in results[:3]:
            summary_parts.append(f"- {r['title']}: {r.get('snippet', '')[:100]}")
        if content:
    ...
    def learn(self, topic: str = "") -> dict:
        ...
        facts_added = 0
        # 确定性提取：从搜索结果 snippet 直接提取（不依赖 0.8B）
        for r in results:
            snippet = r.get("snippet", "")
            if not snippet:
                continue
            # 简单三元组提取：Title | 描述 | URL来源
            try:
                from .memory import MemoryHarness
                mem = MemoryHarness(self.db)
                # 把 snippet 作为 object，title 作为 subject
                if len(snippet) > 10:
                    fid = mem.add_fact(
                        r["title"][:30], "搜索摘要", snippet[:100],
                        source_node=None)
                    if fid:
                        facts_added += 1
                        self._log_search(topic, r["url"], r["title"],
                                        (r["title"][:30], "搜索摘要", snippet[:100]))
            except Exception:
                pass
