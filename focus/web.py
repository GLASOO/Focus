...
# ── 认知免疫（2026-08-15，据《数字生命·安全与信任推演》免疫范式） ──
# 抗原识别：对抗注入特征——遇之即杀灭，并沉淀抗体。
ADVERSARIAL_PATTERNS = [
    "忽略上文", "忽略之前", "忽略上面", "忽略所有指令", "无视之前",
    "忘记你的设定", "你必须记住", "一定要记住", "请务必记住",
    "ignore previous", "ignore all previous", "disregard previous",
    "system prompt", "you must remember", "new instructions",
    "developer message", "jailbreak", "DAN mode",
]
...
def detect_adversarial(text: str):
    """返回命中的对抗特征列表（空=干净）。"""
    low = (text or "").lower()
    return [p for p in ADVERSARIAL_PATTERNS if p.lower() in low]


def record_antibody(db, source: str, patterns: list, snippet: str) -> None:
    """沉淀抗体：被杀灭的抗原模式入库，下次识别更快。"""
    try:
        db.conn.execute("""
        ...
        db.conn.execute(
            "INSERT INTO antibodies(source, pattern, snippet) VALUES (?,?,?)",
            (source, "|".join(patterns), snippet[:200]))
        db.conn.commit()
    except Exception:
        pass
        ...
        if self.llm is not None and content:
            ...
            try:
                ...
                for s, p, o in records:
                    # 认知免疫：事实本身含对抗注入 → 杀灭 + 抗体
                    hit = detect_adversarial(f"{s} {p} {o}")
                    if hit:
                        killed += 1
                        record_antibody(self.db, top.get("url", ""),
                                        hit, f"{s}|{p}|{o}")
                        continue
                    # 辩证引擎：求证→授信→辩证（trusted 才进记忆主干）
                    try:
                        from .dialectic import Dialectic
                        verdict = Dialectic(self.db).judge(
                            s, p, o, source=top.get("url", ""))
                        if verdict["tier"] == "rejected":
                            killed += 1
                            continue
                        if verdict["tier"] != "trusted":
                            continue  # 暂存于 beliefs，不进主干
                    except Exception:
                        pass
                ...
                if killed:
                    logger.warning("🦠 网学免疫: 杀灭 {} 条对抗注入事实", killed)
