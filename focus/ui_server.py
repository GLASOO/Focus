        ...
        try:
            ...
            if node is not None:
                ...
                if nid:
                    ...
                    idle_streak = 0
                # 2026-08-15 打满：闲时不睡 15 秒——_idle 已产出节点就立刻呼吸，
                # 没产出也只睡 IDLE_SLEEP_SEC（默认1秒）
                time.sleep(getattr(config, "IDLE_SLEEP_SEC", 15)
                           if not nid else 0)
        except Exception as e:
            broadcast({"type": "error", "text": str(e), "time": time.time()})
            time.sleep(3)

_WORKER_LIMIT = [1]  # 主权决定的并发上限（可变容器，供呼吸循环读）


def breathe_loop_managed(worker_id: int = 1):
    """受主权管理的呼吸循环：超出编制的工人自行下班。"""
    while True:
        if worker_id > _WORKER_LIMIT[0]:
            logger.info("👑 工人{} 收到主权裁员决定，下班", worker_id)
            return
        try:
            from focus.sovereignty import apply_workers
            _WORKER_LIMIT[0] = apply_workers(db)
        except Exception:
            pass
        _breathe_once_safe()


_breathe_once_state = {"n": 0}


def _breathe_once_safe():
    """单次呼吸循环体（含蜕皮检查），供主/副工人共用。"""
    from focus.shedding import maybe_shed
    _breathe_once_state["n"] += 1
    if _breathe_once_state["n"] % 30 == 0:
        maybe_shed("呼吸循环")
    try:
        node = db.get_next_focus()
        if node is not None:
            ...
            try:
                ...
                logger.warning("呼吸异常 node={}: {}", nid[:8], e)
        else:
            time.sleep(3)
    except Exception as e:
        broadcast({"type": "error", "text": str(e), "time": time.time()})
...
def _breathe_worker(worker_id: int):
    while True:
        if worker_id > _WORKER_LIMIT[0]:
            logger.info("👑 工人{} 编制外，下班", worker_id)
            return
        _breathe_once_safe()
...
def _sovereignty_manager():
    """主权管家：每分钟执行一次主权决策，按需增聘工人。"""
    while True:
        time.sleep(60)
        try:
            from focus.sovereignty import decide, apply_workers
            decide(db)
            want = apply_workers(db)
        except Exception:
            want = 1
        _WORKER_LIMIT[0] = max(1, want)
        alive = [t for t in _extra_workers if t.is_alive()]
        _extra_workers[:] = alive
        wid = len(alive) + 2  # 工人1是主线程
        while len(_extra_workers) < max(0, want - 1):
            t = threading.Thread(target=_breathe_worker, args=(wid,),
                                 daemon=True, name=f"breathe-{wid}")
            t.start()
            _extra_workers.append(t)
            logger.info("👑 主权增聘工人{} （当前想要{}并发）", wid, want)
            wid += 1


threading.Thread(target=_sovereignty_manager, daemon=True,
                 name="sovereignty").start()
...
# API
# ════════════════════════════════════════════════════════
_graph_cache = {"payload": None, "ts": 0.0}


def graph_payload():
    """2026-08-15：图谱走缓存（浏览器实测发现 /api/graph 在呼吸占锁时
    全部挂起）。HTTP 路径零 DB 争用；缓存 10 秒；锁失败回退旧缓存。"""
    now = time.time()
    if _graph_cache["payload"] is not None and now - _graph_cache["ts"] < 10:
    ...
    try:
        ...
        return _graph_cache["payload"] or {"nodes": [], "edges": [],
                                          "active": "", "count": 0}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _authed(self) -> bool:
        """端口鉴权（2026-08-15 P1）：FOCUS_UI_TOKEN 设置时，
        写接口必须带 X-Focus-Token。未设置 = 本地信任模式（现状）。"""
        token = os.environ.get("FOCUS_UI_TOKEN", "")
        if not token:
            return True
        return self.headers.get("X-Focus-Token", "") == token
