...
class DMN:
    ...
    _DREAM_EVERY_SEC = getattr(config, "DREAM_EVERY_SEC", 120)  # 战役1提速
    ...
    def dream(self) -> dict:
        ...
        now = _time.time()
        if now - self._last_dream < self._DREAM_EVERY_SEC:
            return {"skipped": True}
        ...
        fixed = self._memory.dedupe_active()
        extracted = 0
        ...
        try:
            ...
            pass
        logger.info("💤 Dreaming 完成: 提取{} 事实, wiki {} 页, 矛盾自愈 {}",
                    extracted, pages, fixed)
        try:  # 战役3：事件溯源
    ...
    def _ask_llm_priority(self, node: dict) -> str:
        ...
        typ = node.get("type", "")
        prompt = (
            "你是Focus Agent的DMN（默认模式网络），负责后台整理Graph。\n"
            f"节点类型: {typ}\n节点brief: {brief}\n"
