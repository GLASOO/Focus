...
def test_decide_scales_with_pending(db):
    ...
    # 打满后 1 + depth//3，21 待办 → 8，封顶 MAX_WORKERS=4
    assert gov["desired_workers"] == sovereignty.max_workers()
    assert gov["depth"] >= 20
    assert sovereignty.apply_workers(db) == sovereignty.max_workers()
...
def test_pressure_throttle_retreat(db, monkeypatch):
    """内存压力 ≥ 撤退线 → 油门 retreat，生长闸收紧，打满关闭。"""
    import focus.hygiene as H
    from focus import config
    monkeypatch.setattr(H, "mem_pressure", lambda: 0.97)
    gov = sovereignty.decide(db)
    assert gov["throttle"] == "retreat"
    assert config.GROWTH_HOURLY_CAP == 30
    assert config.FULL_THROTTLE is False


def test_pressure_throttle_full(db, monkeypatch):
    """压力低 → 打满全速。"""
    import focus.hygiene as H
    from focus import config
    monkeypatch.setattr(H, "mem_pressure", lambda: 0.4)
    gov = sovereignty.decide(db)
    assert gov["throttle"] == "full"
    assert config.FULL_THROTTLE is True
